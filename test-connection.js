...
async function runTest() {
  console.log(chalk.blue('Starting API Test...'));
  
  const testDomain = 'google.com';
  console.log(`Checking single domain: ${testDomain}`);
  
  const result = await checkDomainAvailability(testDomain);
  
  if (result.error) {
    console.log(chalk.red('✖ API call failed! Check your credentials and GODADDY_API_URL in .env.'));
    process.exit(1);
  }
  ...
  console.log('Bulk Result:', JSON.stringify(bulkResults, null, 2));
